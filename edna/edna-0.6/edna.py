...
class Server(mixin, BaseHTTPServer.HTTPServer):
  def __init__(self, fname):
    ...
    PLAYER = config.get('server', 'player')
...
#            info = FileInfo(fullpath)
  ...
  def display_playing(self, songs):
    self.send_response(200)
    self.send_header("Content-Type", 'text/html')
    self.end_headers()
    ...
    d.name = songs
    data['songs'].append(d)
    self.server.playing_template.generate(self.wfile, data)
  ...
  def serve_file(self, name, fullpath, url, range=None):
    ...
    if any_extensions.has_key(ext):
      ...
      if name == 'all.m3u' or name == 'allrecursive.m3u' or \
        ...
        newbase, ext = os.path.splitext(base)
        if extensions.has_key(string.lower(ext)):
          print "Build: "+ base
          subprocess.Popen([PLAYER, fullpath])
          self.display_playing(newbase)
          return
        else:
